...
class Emotion_SSL_Dataset:
    def __init__(self,
        ...
        self.num_classes = num_classes
        self.data_dir = data_dir
        self.train_json_path = getattr(args, "train_json_path", "")
        self.test_json_path = getattr(args, "test_json_path", "")
        self.data_files_dir = getattr(args, "data_files_dir", "")
        crop_size = 224
        self.transform = get_emotion_transform(mean[name], std[name], crop_size, train)
    ...
    def get_data(self, svhn_extra=True):
        ...
        if self.data_dir is None:
            ...
            exit(-1)
        if self.train:
            json_path = self.train_json_path if self.train_json_path else os.path.join(self.data_dir, 'train.json')
        else:
            json_path = self.test_json_path if self.test_json_path else os.path.join(self.data_dir, 'test.json')
        with open(json_path, "r") as f:
            anno = json.load(f)

        data_root = self.data_files_dir if self.data_files_dir else self.data_dir
        data_subdir = os.path.join(data_root, 'data')
        img = []
        text = []
        img_label = []
        text_label = []
        label = []
        for id in anno.keys():
            # Image.open(os.path.join(self.data_dir,'data',id+'.jpg'))
            img.append(np.array(Image.open(os.path.join(data_subdir, id + '.jpg')).convert('RGB')))
            with open(os.path.join(data_subdir, id + '.txt'), 'r', encoding='ISO-8859-1') as file:
                text.append(file.read())
            # text_path.append(os.path.join(self.data_dir,'data',id+'.txt'))
            img_label.append(anno[id]['img_label'])
            text_label.append(anno[id]['text_label'])
            label.append(anno[id]['label'])

        return img, text, img_label, text_label, label
    ...
    def get_dset(self, is_ulb=False,
        ...
        img, text, img_label, text_label, label = self.get_data()
        num_classes = self.num_classes
        transform = self.transform

        return BasicDataset(self.alg, img, text, label, num_classes, transform,
                            is_ulb, strong_transform, onehot)
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        if self.alg == 'fullysupervised':
            ...
            # data, targets = self.get_data()
            img, text, img_label, text_label, label = self.get_data()
            lb_img, lb_text, lb_targets, ulb_img, ulb_text, ulb_targets = split_ssl_data(self.args, img, text, img_label, text_label, label,
                                                                        num_labels, self.num_classes,
                                                                        index, include_lb_to_ulb)
